...
export async function download( url: string, filePath: string, showProgress = false, name = '', headers?: Record<string, string> ) {
	...
	if (headers && Object.keys(headers).length > 0) {
		...
		const showDownloadProgress = showProgress && typeof process.stdout.clearLine === 'function' && !isNaN(totalSize);
