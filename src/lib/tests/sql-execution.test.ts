...
describe('SQL Execution with Temporary Files', () => {
	...
	it('should handle DB_NAME constant replacement', () => {
		const sqlWithDbName = "SELECT * FROM wp_options WHERE option_value LIKE CONCAT('%', DB_NAME, '%')";
		const result = convertMySqlToSqlite(sqlWithDbName);
		expect(result).toBe("SELECT * FROM wp_options WHERE option_value LIKE CONCAT('%', 'wordpress', '%')");
	});
}); 