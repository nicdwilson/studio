	...
	if ( ! isInstalled ) {
		return (
			<div className="space-y-8">
				...
				<Card className="p-6">
					<h3 className="text-lg font-medium text-gray-900 mb-4">{ __( 'Installation' ) }</h3>
					<p className="text-gray-600 mb-4">
						{ __(
							'The setup process requires the command line. Click the button below to launch the installation script in a terminal window.'
						) }
					</p>
						...
						{ showInstallInstructions && (
							<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
								...
								<ol className="text-blue-800 space-y-2 text-sm">
									...
									<li>
										2.{ ' ' }
										{ __( 'The first prompt is for your Mac password (for system path access)' ) }
									</li>
									<li>
										3.{ ' ' }
										{ __(
											'The second prompt is for the SSH key password (optional but recommended)'
										) }
									</li>
				...
				<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
					...
					<ul className="text-yellow-800 space-y-2">
						<li className="flex items-start">
							...
							<span>
								{ __(
									'Jurassic Tube is essential for testing WooCommerce Payments, webhooks, and Jetpack connections'
								) }
							</span>
						</li>
						<li className="flex items-start">
	...
	return (
		<div className="space-y-8">
			<div className="max-w-3xl px-8">
				...
				<p className="text-gray-600 mb-6">
					{ __(
						'Configure your Jurassic Tube connection to enable tunneling for payment testing and webhooks.'
					) }
				</p>
			</div>
			...
			<Card className="p-6">
				...
				<div className="space-y-4">
					...
					/>
					<Button variant="secondary" onClick={ saveUsername } disabled={ ! wpUsername }>
						{ __( 'Save Username' ) }
					</Button>
				</div>
				...
				<div className="mt-6 space-y-4">
					...
					/>
					<Button variant="secondary" onClick={ saveSubdomain } disabled={ ! subdomain }>
						{ __( 'Save Subdomain' ) }
					</Button>
				</div>
			...
			{ /* Connection Status */ }
			<Card className="p-6">
				<h3 className="text-lg font-medium text-gray-900 mb-4">{ __( 'Connection Status' ) }</h3>
				...
				<div className="flex items-center justify-between">
					...
					<div className="flex gap-2">
						{ ! isConnected ? (
							...
							<Button variant="secondary" onClick={ disconnectJurassicTube }>
								{ __( 'Disconnect' ) }
							</Button>
						) }
					</div>
