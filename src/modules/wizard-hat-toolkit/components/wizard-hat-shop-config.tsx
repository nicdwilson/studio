	...
	return (
		<div className="space-y-8">
			<div className="max-w-3xl px-8">
				...
				<p className="text-gray-600 mb-6">
					{ __(
						'Quickly switch your WooCommerce store settings between different geographic regions. This updates currency, address format, weight units, and other locale-specific settings.'
					) }
				</p>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{ SHOP_CONFIGS.map( ( config ) => (
					...
					>
						<h3 className="text-lg font-medium text-gray-900 mb-4">{ config.name }</h3>
						<div className="space-y-2 mb-6 text-sm text-gray-600">
							...
							<div>
								<strong>{ __( 'Dimension Unit' ) }:</strong>{ ' ' }
								{ config.options.woocommerce_dimension_unit }
							</div>
							<div>
			...
			<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
				...
				<ul className="text-yellow-800 space-y-2">
					...
					<li className="flex items-start">
						...
						<span>
							{ __(
								'Changes include currency, address format, weight units, and dimension units'
							) }
						</span>
					</li>
					<li className="flex items-start">
						<span className="font-medium mr-2">•</span>
						<span>
							{ __( 'Make sure to test your checkout process after switching configurations' ) }
						</span>
					</li>
				</ul>
			</div>
