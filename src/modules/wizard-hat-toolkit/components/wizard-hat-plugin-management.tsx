...
const QUICK_INSTALL_MARKETPLACE_PLUGINS: PluginOption[] = [
	...
	{ label: 'AutomateWoo', value: 'automatewoo', type: 'premium', repository: 'https://github.com/woocommerce/all-plugins' },
];
...
export function WizardHatPluginManagement() {
	...
	const installSelectedPlugins = async () => {
		...
		try {
			...
			const allPlugins = [...QUICK_INSTALL_PLUGINS, ...QUICK_INSTALL_MARKETPLACE_PLUGINS, ...allPremiumPlugins];
			const pluginsToInstall = allPlugins.filter(plugin => selectedPlugins.includes(plugin.value));
			
			setInstallationLog(prev => [...prev, `🚀 Starting installation of ${pluginsToInstall.length} plugins...`]);
	...
	return (
		<div className="space-y-8">
			<div className="max-w-3xl px-8">
				...
				<p className="text-gray-600 mb-6">
					{__('Install and manage WooCommerce plugins. Configure your GitHub token in the Tools tab for marketplace plugins.')}
				</p>
			</div>
			...
			<div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
				...
				<ul className="text-blue-800 space-y-2">
					<li className="flex items-start">
						...
						<span>{__('WordPress.org plugins are installed from the official repository')}</span>
					</li>
					<li className="flex items-start">
						<span className="font-medium mr-2">•</span>
						<span>{__('Marketplace plugins require a valid GitHub token (configure in Tools tab)')}</span>
					</li>
					<li className="flex items-start">
						<span className="font-medium mr-2">•</span>
